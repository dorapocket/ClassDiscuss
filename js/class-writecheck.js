...
function validate_form(thisform) {
    with (thisform) {
        if (validate_required(Score, "请评价综合评分~~") == false) { vallscore.focus(); return false; }
        if (validate_required(vdifficulty, "课程难度还未评价！") == false) { vdifficulty.focus(); return false; }
        if (validate_required(vgetsinfo, "请评价收获程度~~") == false) { vgetsinfo.focus(); return false; }
        if (validate_required(vbetterscore, "老师给分好坏未评价！") == false) { vbetterscore.focus(); return false; }
        if (validate_required(vhomework, "请评价作业多少~~") == false) { vhomework.focus(); return false; }
    }
}
